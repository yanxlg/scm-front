...
import { addPddHotTask, IPddHotTaskParams, queryCategory, querySortCondition, queryTaskDetail } from '@/services/task';
...
class _HotGather extends Form.BaseForm<IHotGatherProps, IHotGatherState> {
    ...
    private querySortCondition() {
        querySortCondition().then(({ data: { sortCondition } }) => {
            this.setState({
                sortLoading: false,
    ...
    render() {
        ...
        const { gatherLoading, groundLoading, queryLoading, taskType, failTimes, successTimes, pddCategory = [], categoryLoading, sortLoading, sortCondition = [] } = this.state;
        const childCategory = pddCategory.find((category) => {
        ...
        return (
            <Spin spinning={queryLoading} tip="Loading...">
                ...
                <Form className="form-help-absolute" layout="inline" autoComplete={'off'}>
                    <Form.Item className="block form-item" validateTrigger={'onBlur'} form={form}
                               name="task_name" label="任务名称">
                        <Input className="input-default"/>
                    </Form.Item>
                    <Card className="config-card" title={<span className="ant-form-item-required">任务范围：</span>}>
                        <Form.Item validateTrigger={'onBlur'} form={form} name="range"
                            ...
                            <Radio.Group>
                                ...
                                <div className="block form-item">
                                    ...
                                    <Form.Item className="vertical-middle" validateTrigger={'onBlur'} form={form}
                                               label="店铺ID" name="shopId" rules={[{
                                        required: range === 'shop',
                                        message: '请输入店铺ID',
                                    }]}>
                                        <InputNumber min={0} placeholder={'请输入'} className="input-default input-handler"
                                                     formatter={numberFormatter} disabled={range !== TaskRange.store}/>
                                    </Form.Item>
                                </div>
                            </Radio.Group>
                        </Form.Item>
                            ...
                            <Select loading={categoryLoading} className="select-default"
                                    onChange={this.onCategoryChange}>
                                {
                                    pddCategory.map((category) => {
                                        return <Option key={category.platform_cate_id}
                                                       value={category.platform_cate_id}>{category.platform_cate_name}</Option>;
                                    })
                                }
                            </Select>
                        </Form.Item>
                            ...
                            <Select loading={sortLoading} className="select-default">
                                {
                                    sortCondition.map((sort) => {
                                        return <Option key={sort.value} value={sort.value}>{sort.display}</Option>;
                                    })
                                }
                            </Select>
                        </Form.Item>
                        <Form.Item className="block form-item" validateTrigger={'onBlur'} form={form} name="keywords"
                    ...
                    <Card className="config-card" title="设置商品条件：">
                        <div className="block">
                            <div className="inline-block">
                                <Form.Item className="margin-none" validateTrigger={'onBlur'} form={form}
                                           name="sales_volume_min" label="销&emsp;&emsp;量">
                                    <InputNumber min={0} className="input-small input-handler"
                        ...
                        <div className="block form-item">
                            <Form.Item validateTrigger={'onBlur'} form={form} name="grab_page_count" label="爬取页数">
                                <InputNumber min={0} className="config-input-pages input-handler"
                                             formatter={numberFormatter} width={200}/>
                            </Form.Item>
                            <Form.Item validateTrigger={'onBlur'} form={form} name="grab_count_max" label={
                                <span>爬取数量<Tooltip placement="bottom" title="各指定类目筛选前可爬取的最大数量"><Icon
                                    type="question-circle" className="config-ques"/></Tooltip></span>
                            }>
                                <InputNumber min={0} className="config-input-count input-handler"
                                             formatter={numberFormatter}/>
                            </Form.Item>
                        </div>
                    </Card>
                        ...
                        <Form.Item validateTrigger={'onBlur'} form={form} name="task_type" initialValue={TaskType.once}>
                            <Radio.Group>
                                <div className="block">
                                    ...
                                    <Form.Item className="vertical-middle" validateTrigger={'onBlur'} form={form}
                                               label="开始时间" name="onceStartTime">
                                        <DatePicker showTime={true} placeholder={'立即开始'}
                                ...
                                <div className="form-item">
                                    ...
                                    <Form.Item className="vertical-middle" validateTrigger={'onBlur'} form={form}
                                               label="开始时间" name="timerStartTime">
                                        <DatePicker showTime={true} disabled={task_type !== TaskType.interval}/>
                                    </Form.Item>
                                    <Form.Item className="vertical-middle" validateTrigger={'onBlur'} form={form}
                                               label="结束时间" name="task_end_time">
                                        <DatePicker showTime={true} disabled={task_type !== TaskType.interval}/>
                                    </Form.Item>
                                    <Form.Item className="vertical-middle" validateTrigger={'onBlur'} form={form}
                                               label="任务间隔" name="taskIntervalType" initialValue={TaskIntervalType.day}>
                                        <Radio.Group disabled={task_type !== TaskType.interval}>
                                            <Radio value={TaskIntervalType.day}>
                                                <Form.Item className="vertical-middle" validateTrigger={'onBlur'}
                                                           form={form} name="day">
                                                    <InputNumber min={0} className="input-small input-handler"
                                                                 formatter={numberFormatter}
                                                                 disabled={task_type !== TaskType.interval || taskIntervalType !== TaskIntervalType.day}/>
                                                </Form.Item>
                                                天
                                            </Radio>
                                            <Radio value={TaskIntervalType.second}>
                                                <Form.Item className="vertical-middle" validateTrigger={'onBlur'}
                                                           form={form} name="second">
                                                    <InputNumber min={0} className="input-small input-handler"
                                                                 formatter={numberFormatter}
                                                                 disabled={task_type !== TaskType.interval || taskIntervalType !== TaskIntervalType.second}/>
                                                </Form.Item>
                                                秒
                                            </Radio>
                                        </Radio.Group>
                    ...
                    <div className="form-item">
                        <Button loading={gatherLoading} type="primary" className="btn-default"
                                onClick={this.onStartGather}>
                            {
                                edit ? '创建新任务' : '开始采集'
                            }
                        </Button>
                        <Button loading={groundLoading} type="primary" className="btn-default"
