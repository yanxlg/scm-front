...
import { Modal, Form, Input, Row, Col, Select, Upload, Button, message } from 'antd';
import { getCatagoryList, exportAllSkuImages, uploadGoodsPic, putGoodsEdit, IGoodsEditData } from '@/services/goods';
...
const GoodsEditModal: React.FC<IProps> = ({ visible, currentGoodsInfo, onCancel, onReload }) => {
    ...
    // 判断商品数据是否改变
    const hasChangeGoodsInfo = useCallback((formData, currentGoodsInfo) => {
        const {
            title,
            description,
            first_catagory,
    ...
    // 获取商品编辑数据
    const getGoodsEditData = useCallback((formData, currentGoodsInfo) => {
        const { title, description, first_catagory, second_catagory, third_catagory } = formData;
            ...
            >
                <Form form={form} className={styles.modalContent}>
                    ...
                    <div className={styles.btns}>
                        ...
                        >
                            <Button loading={uploading} icon={<Icons type="scm-upload" />}>批量上传</Button>
                        </Upload>
                        <LoadingButton
    ...
    }, [visible, allCatagoryList, imgList, secondCatagoryList, thirdCatagoryList, uploading, confirmLoading]);
};
